...
namespace EllipseUtils
{
	...
	public:
		...
		static EllipseParameters<tFloat> LeastSquaresFitEllipse2(const std::vector<tFloat>& posX, const std::vector<tFloat>& posY)
		{
			return LeastSquaresFitEllipse2((std::min)(posX.size(), posY.size()), posX, posY);
		}

		static EllipseParameters<tFloat> LeastSquaresFitEllipse2(size_t count, const std::vector<tFloat>& posX, const std::vector<tFloat>& posY)
		{
		...
		static const Eigen::Matrix<tFloat, 3, 3> MatrixConstant;// = (Eigen::Matrix<tFloat, 3, 3>() << 0, 0, -0.5, 0, 1, 0, -0.5, 0, 0).finished();
		...
		static EllipseParameters<tFloat> LeastSquaresFitEllipse(const tGetData& gd)
		{
			...
			Eigen::Matrix<tFloat, 3, 1> eigenVec;
			eigenVec = eigenSolver.eigenvectors().block<3, 1>(0, indexPositiveEigenValue).real();
			...
			EllipseAlgebraicParameters<tFloat> ep
			{
			 ...
			 eigenVec(1) * sx*sy,
			 eigenVec(2) * sx*sx,
			 -2 * eigenVec(0) * sy*sy*mx - eigenVec(1) * sx*sy*my + tv0(3 - 3) * sx*sy*sy,
			 -eigenVec(1) * sx*sy*mx - 2 * eigenVec(2) * sx*sx*my + tv0(4 - 3) * sx*sx*sy,
